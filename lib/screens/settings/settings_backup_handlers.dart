        ...
        if (hasVaultData && context.mounted) {
          ...
          if (recovered != true) {
            if (context.mounted) {
              ApexSnackBar.show(
                context,
                Localizations.localeOf(context).languageCode == 'ar' ? 'تم إلغاء الاستيراد' : 'Import cancelled',
      ...
      if (hasVaultData && context.mounted) {
        ...
        if (recovered != true) {
          if (context.mounted) {
            ApexSnackBar.show(
              context,
              lang == 'ar' ? 'تم إلغاء الاستعادة' : 'Restore cancelled',
              type: SnackBarType.warning,
            );
          }
          return;
      ...
      if (hasVaultData && context.mounted) {
        ...
        if (recovered != true) {
          if (context.mounted) {
            ApexSnackBar.show(
              context,
              lang == 'ar' ? 'تم إلغاء الاستيراد' : 'Import cancelled',
              type: SnackBarType.warning,
            );
          }
          return;
      ...
      if (localCount == 0) {
        ...
        if (context.mounted) {
          ...
          if (lockedNotes.isNotEmpty) {
            ...
            ApexSnackBar.show(context, "${notesToImport.length} ${l10n.importedSuccessfully}", type: SnackBarType.success);
          }
        }
        return;
      }
      ...
      if (context.mounted) {
        ...
        if (context.mounted) {
          ...
          if (lockedNotes.isNotEmpty) {
            ...
            ApexSnackBar.show(
              context, 
              action == 'replace' ? l10n.restoredSuccessfully : l10n.mergedSuccessfully,
              type: SnackBarType.success,
            );
          }
        }
