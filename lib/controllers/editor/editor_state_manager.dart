...
class EditorStateManager {
  ...
  /// Reset state to original snapshot
  /// 
  /// Discards all changes and reverts to the last saved state.
  /// 
  /// **Use Cases:**
  ...
  void resetToSnapshot() {
    ...
    customTitle = originalTitle.isNotEmpty ? originalTitle : null;
    colorIndex = originalColorIndex;
    reminderDateTime = originalReminderDateTime;
    recurrenceRule = originalRecurrenceRule;
    isDirty = false;
  ...
  /// Update color and mark as dirty
  /// 
  /// Convenience method to update color and set dirty flag.
  /// 
  /// **Parameters:**
  /// - `newColorIndex`: New color index
  void updateColor(int newColorIndex) {
    colorIndex = newColorIndex;
    if (newColorIndex != originalColorIndex) {
      markDirty();
    }
  }
  
  /// Update reminder and mark as dirty
  /// 
  /// Convenience method to update reminder and set dirty flag.
  /// 
  /// **Parameters:**
  /// - `newReminderDateTime`: New reminder date/time
  /// - `newRecurrenceRule`: New recurrence rule
  void updateReminder({
    ...
    if (newReminderDateTime != originalReminderDateTime ||
        newRecurrenceRule != originalRecurrenceRule) {
      markDirty();
    }
  }
