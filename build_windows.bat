...
flutter analyze lib/ --no-pub > analyze_out.tmp 2>&1
...
flutter build windows --release > build_out.tmp 2>&1
type build_out.tmp | findstr "Built\|error\|Error\|FAILED"
...
"%ISCC%" sinan_note_setup.iss > inno_out.tmp 2>&1
type inno_out.tmp | findstr "Successful\|SinanNote_Setup\|Error"
type inno_out.tmp | findstr "Successful" >nul 2>&1
if errorlevel 1 (
